...
3. When you access `homelab.local:8080` from outside, the client routes traffic through Twingate's relay to the connector, which forwards it to the local service
...
4. Define Resources (e.g. `homelab.local` or a whole CIDR range) to control what's accessible
